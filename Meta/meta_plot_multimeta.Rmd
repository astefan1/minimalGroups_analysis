...
p <- ggplot(plotdata, aes(x = b_Intercept, y = biasMeasure, fill = ideologyMeasure, color = ideologyMeasure)) +
  ...
  stat_halfeye(.width = c(.9, .95), slab_alpha=0.4) +
  scale_fill_manual(values = alpha(c("#56B4E9", "#D55E00")), labels = c("Issue-based", "Self-reported")) +
  scale_color_manual(values = alpha(c("#56B4E9", "#D55E00")), labels = c("Issue-based", "Self-reported")) +
  theme_minimal() +
  labs(
