...
p <- ggplot(plotdata, aes(x = b_Intercept, y = biasMeasure, fill = ideologyMeasure, color = ideologyMeasure)) +
  ...
  stat_slab(alpha = 0.4) +
  stat_pointinterval(data = plotdata[plotdata$ideologyMeasure == "selfReported", ], position = position_nudge(y = -0.1), .width = c(.9, .95)) +
  stat_pointinterval(data = plotdata[plotdata$ideologyMeasure == "issueBased", ], position = position_nudge(y = -0.05), .width = c(.9, .95)) +
  scale_fill_manual(values = alpha(c("#56B4E9", "#D55E00")), labels = c("Issue-based ideology", "Self-reported ideology")) +
  scale_color_manual(values = alpha(c("#56B4E9", "#D55E00")), labels = c("Issue-based ideology", "Self-reported ideology")) +
  theme_minimal() +
  labs(
  ...
  theme(text=element_text(size=20),
        ...
        legend.position.inside = c(0.84, 1)) +
  xlim(-0.3, 0.75) +
...
ggsave("./model-fits/nhbMeta_multimeta-forest.pdf", device = "pdf", width=10.250000, height=6.666667)
ggsave("./model-fits/nhbMeta_multimeta-forest.jpg", device = "jpeg", width=10.250000, height=6.666667)
